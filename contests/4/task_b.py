...
def generate(num_players: int) -> dict[str, Any]:
    building_costs = [random.randint(low, high) for low, high in
                      BUILDING_COST_RANGES[:num_players]]

    initial_cards_count = INITIAL_CARDS_COUNT[num_players]
    card_deck = list(range(1, 43))
    random.shuffle(card_deck)
    players_cards = [card_deck.pop(0) for _ in
                     range(initial_cards_count * num_players)]

    random.shuffle(PLAYER_COLORS)
    players = [{'color': color, 'cards': players_cards[i::num_players]}
